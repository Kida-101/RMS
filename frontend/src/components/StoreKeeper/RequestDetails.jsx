...
const RequestDetails = ({ data, closePopup }) => {
  return (
    <div className="fixed inset-0 bg-black/50 group  flex items-center justify-center ">
      {" "}
      <div key={data.supplier.id} className="group relative w-full sm:w-80 m-2">
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-b from-black shadow-2xl hover:shadow-cyan-500/25">
            ...
            <div className="relative">
              <div className="mt-2 flex items-baseline gap-2">
                <span className="text-2xl font-bold text-black">
                  {data.supplier.stockType}
                </span>
              </div>
              <div className="mt-4">
                <h3 className="text-lg font-semibold text-gray-800">Items</h3>
                <ul className="list-disc pl-4 text-[17px] text-gray-600">
                  {data.items.map((item, index) => (
                    <li
                      key={index}
                    >{`Material: ${item.material} -  Quantity: ${item.quantity}`}</li>
                  ))}
                </ul>
              </div>
              <hr />
              ...
              <div className="mt-2">
                ...
                <div className="flex flex-col gap-2">
                  {data?.supplier?.contacts.map((contact, index) => (
